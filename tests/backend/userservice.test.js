...
describe("Make User service", () => {
  ...
  describe("create user", () => {
    ...
    test("check for the name is empty", async () => {
      await expect(
        userService.createUser({ email: "john@example.com" }),
  ...
  describe("GetUser By id", () => {
    ...
    test("check the user with blank id", async () => {
      await expect(userService.getUserById()).rejects.toMatchObject({
    ...
    test("check the user id is not found", async () => {
      await expect(userService.getUserById("u1")).rejects.toMatchObject({
  ...
  describe("Update user", () => {
    ...
    test("checking the user is not found", async () => {
      await expect(
        userService.updateUser("t1", { name: "" }),
      ).rejects.toMatchObject({
        message: "User not found",
        status: 404,
      });
    ...
    test("user without any input field", async () => {
      await expect(
        userService.updateUser(null, { invalid: "field" }),
      ).rejects.toMatchObject({
        status: 400,
      });
    });
  ...
  describe("Delete the user ", () => {
    ...
    test("Checking the user id not found", async () => {
      await expect(userService.deleteUser()).rejects.toMatchObject({
        message: "User ID is required",
        status: 400,
      });
    });
    ...
    test("Checking the user is not found", async () => {
      userRepo.findById.mockResolvedValue(null);
      await expect(userService.deleteUser("u999")).rejects.toMatchObject({
        message: "User not found",
        status: 404,
      });
