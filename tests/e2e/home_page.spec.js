...
test.describe("Main Page", () => {
  ...
  test("Download App link ", async ({ page }) => {
    ...
    await expect(appPage).toHaveURL(
      "https://apps.apple.com/in/app/find-banquet/id1547801752",
    );

    const [androidPage] = await Promise.all([
      page.context().waitForEvent("page"),
      page.getByRole("link", { name: "Google Play Get it on Android" }).click(),
    ]);
    await expect(androidPage).toHaveURL(
      "https://play.google.com/store/apps/details?id=com.findbanquet&hl=en_IN",
    );
  });
});
