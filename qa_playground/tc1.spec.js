...
test('Input Field Automation', async ({ page }) => {
  ...
  await expect(page.locator('#readonlyInput')).toHaveValue(
...
test('TC02: Verify input placeholder disappears on typing', async ({
  page,
}) => {
...
test('TC03: Verify keyboard tab triggers focus change after append', async ({
  page,
}) => {
...
test('TC04: Verify appended text value is retained in the field', async ({
  page,
}) => {
  const input = await page.getByTestId('input-append-text');
...
test('TC05: Verify text present inside input field matches expected value', async ({
  page,
}) => {
...
test('TC06: Verify getAttribute returns the correct input value', async ({
  page,
}) => {
  const input = await page.getByTestId('input-verify-text');
...
test('TC07: Verify input field text can be cleared successfully', async ({
  page,
}) => {
...
test('TC08: Verify field is empty after executing clear action', async ({
  page,
}) => {
  const input = await page.getByTestId('input-clear-text');
  await input.fill('This is me');
  await input.clear();
  await expect(input).toHaveValue('');
});

test('TC09: Verify disabled input field cannot be edited by user', async ({
  page,
}) => {
...
test('TC11: Verify readonly input field does not accept user typing', async ({
  page,
}) => {
...
test('TC12: Verify getAttribute returns correct readonly attribute value', async ({
  page,
}) => {
  const input = await page.getByTestId('input-readonly');
